...
class Llama:
    ...
    def generate(
        ...
        for cur_pos in range(self.min_prompt_len, total_len):
            logits, self.h_last_real_token_bl, self.h_ave_real_token_bl, self.h_list = (
                self.model.forward(tokens[:, prev_pos:cur_pos], prev_pos)
            )  # Obtain both the output of the Transformer model and the text embedding vectors from the final layer.
            if logprobs:
    ...
    def text_completion(
        ...
        ]  # Define `prompt_tokens` as a protected member variable of the class to facilitate access.
    ...
    def get_text_prompt_token_embedding(self):
        ...
        for batch_idx, pt in enumerate(self._prompt_tokens):  # print tokens number
            if len(pt) == self.min_prompt_len:
                ...
                for temp_prompt_pos in range(1, prompt_pos + 1):
                    ...
                    h_ave_real_token_s_cat = torch.cat(
                        (h_ave_real_token_s_cat, temp_prompt), dim=0
                    )
            ...
            self.h_last_real_token_sl_tensor = torch.stack(
                self.h_last_real_token_sl, dim=0
            )
            ...
            self.h_ave_real_token_sl_tensor = torch.stack(
                self.h_ave_real_token_sl, dim=0
            )
            ...
            self.h_pca_real_token_s = self.matrix_to_vector_pca_rescaled(
                h_ave_real_token_s_cat
            )
            self.h_pca_real_token_sl.append(self.h_pca_real_token_s)
            self.h_pca_real_token_sl_tensor = torch.stack(
                self.h_pca_real_token_sl, dim=0
            )
        ...
        return (
            self.h_last_real_token_bl,
            self.h_ave_real_token_bl,
            self.h_last_real_token_sl_tensor,
            self.h_ave_real_token_sl_tensor,
            self.h_pca_real_token_sl_tensor,
            self.h_mat_real_token_sl,
        )
    ...
    def get_chat_prompt_token_embedding(self):
        ...
        self.h_eis_ave_real_token_sl_tensor = torch.stack(
            self.h_eis_ave_real_token_sl, dim=0
        )
