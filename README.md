...
0. Download [filelists](a hugging face repo to appear) which contains semantic embeddings extracted from Llama and various BERT models. 
    1. `filelists` folder contains exact training information for every dataset along with corresponding semantic embeddings in our experiments. 
0. Create more soft links to facilitate access to common configurations among different `*_VITS` methods. 
    Create soft links to the `configs/` for each `*_VITS` method.   
    ```sh
    ln -s /path/to/configs /path/to/ori_vits/
    ln -s /path/to/configs /path/to/emo_vits/
    ln -s /path/to/configs /path/to/sem_vits/
    ```
    Create soft links to the `filelists/` for each `*_VITS` method.   
    ```sh
