...
0. Install python requirements. Please refer to [requirements.txt](requirements.txt)
    ...
    1. You may also try out below requirements that we extract from our environment if `requirements.txt` does not work for you. 
        ...
        ```
0. Download datasets
    1. Download and extract the LJSpeech dataset from [here](https://keithito.com/LJ-Speech-Dataset/), then rename or use absolute paths to create soft links to your data:
        ```sh
        ln -s /path/to/LJSpeech-1.1/wavs /path/to/vitsGPT/DUMMY1
        ln -s /path/to/LJSpeech-1.1/wavs /path/to/vits/DUMMY1
        ln -s /path/to/LJSpeech-1.1/wavs /path/to/ori_vits/DUMMY1
        ln -s /path/to/LJSpeech-1.1/wavs /path/to/emo_vits/DUMMY1
        ln -s /path/to/LJSpeech-1.1/wavs /path/to/sem_vits/DUMMY1
        ```
    1. Download and extract the EmoV_DB_bea_sem dataset from [here](https://drive.google.com/file/d/1-lbFJ2mw4b8ruFK9RNHvfSTQJXWlJVuS/view?usp=sharing), then rename or create a link to the dataset folder: 
        ```sh
        ln -s /path/to/EmoV_DB_bea_sem/wavs_filtered /path/to/vitsGPT/DUMMY5
        ln -s /path/to/EmoV_DB_bea_sem/wavs_filtered /path/to/vits/DUMMY5
        ln -s /path/to/EmoV_DB_bea_sem/wavs_filtered /path/to/ori_vits/DUMMY5
        ln -s /path/to/EmoV_DB_bea_sem/wavs_filtered /path/to/emo_vits/DUMMY5
        ln -s /path/to/EmoV_DB_bea_sem/wavs_filtered /path/to/sem_vits/DUMMY5
        ```
    1. You can also download EmoV_DB from [here](https://www.openslr.org/115/) and filter it yourself by refering to our  [preprocess_EmoV_DB_bea_filter.py](datasets/preprocess_EmoV_DB_bea_filter.py). We also provide other code for necessary data preprocessing, e.g., downsampling, in this folder. Note that the `gt_test_wav` folder includes all test audios that we have processed to the same sampling rate with those generated by `*-VITS` method. You can also process it by your own if using other dataset.  
...
0. Download [filelists](a hugging face repo to appear) which contains semantic embeddings extracted from Llama and various BERT models.  
    `filelists` folder contains exact training information for every dataset along with corresponding semantic embeddings in our experiments. 
0. Create more soft links to facilitate access to common configurations among different `*_VITS` methods. 
    1. Create soft links to the `configs/` for each `*_VITS` method.   
        ```sh
        ln -s /path/to/configs /path/to/ori_vits/
        ln -s /path/to/configs /path/to/emo_vits/
        ln -s /path/to/configs /path/to/sem_vits/
        ```
    1. Create soft links to the `filelists/` for each `*_VITS` method.   
        ```sh
        ln -s /path/to/filelists /path/to/ori_vits/
        ln -s /path/to/filelists /path/to/emo_vits/
        ln -s /path/to/filelists /path/to/sem_vits/
        ```
0. Build Monotonic Alignment Search and run preprocessing if you use your own datasets.  
    ...
    Note that we have provided preprocessed phonemes for LJSpeech, 1-hour LJSpeech, and EmoV_DB_bea_sem in `filelists` named as `datasetname_audio_text_*_filelist.txt.cleaned`. 

## Extracting Semantic Embeddings 
Note that we have provided all extracted semantic embeddings from Llama or various BERT models in `filelists` named as `datasetname_audio_tokenname_dimension.pt`. 
