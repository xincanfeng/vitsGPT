...
0. Download datasets
    1. Download and extract the LJSpeech dataset from [here](https://keithito.com/LJ-Speech-Dataset/), then rename or use absolute paths to create soft links to your data:
        ...
        ```
    1. Download and extract the EmoV_DB_bea_sem dataset from [here](https://huggingface.co/datasets/xincan/Llama-VITS_data), then rename or create a link to the dataset folder: 
        ```sh
        ...
        ```
    1. You can also download [EmoV_DB](https://www.openslr.org/115/) and filter it yourself by refering to our [preprocess_EmoV_DB_bea_filter.py](datasets/preprocess_EmoV_DB_bea_filter.py). We also provide code for other necessary data preprocessing, e.g., downsampling, in this folder. Note that the `gt_test_wav` folder includes all test audios that we have processed to the same sampling rate with those generated by `{method}_VITS` methods. You can also process it by your own if using other datasets.  
...
0. Create more soft links to facilitate access to common configurations among different `{method}_VITS` methods. 
    1. Create soft links to the `configs/` for each `{method}_VITS` method.   
        ```sh
