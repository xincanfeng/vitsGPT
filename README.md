...
0. Download datasets
    ...
    1. Download and extract our EmoV_DB_bea_sem dataset from [here](https://huggingface.co/datasets/xincan/Llama-VITS_data), then rename or create a link to the dataset folder: 
        ...
        ```
    1. You can also download [EmoV_DB](https://www.openslr.org/115/) and filter it yourself by refering to our [preprocess_EmoV_DB_bea_filter.py](datasets/preprocess_EmoV_DB_bea_filter.py). We also provide code for other necessary data preprocessing, e.g., downsampling, in the folder [datasets](datasets). 
