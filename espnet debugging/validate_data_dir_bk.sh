...
echo "Script is called from: $(pwd)"
...
if ! $no_text; then
  if ! $non_print; then
    ...
    # echo "$0: $data/text contains $n_non_print lines with non-printable characters" &&\
    # exit 1;
    # ”“”
    n_non_print=$(LC_ALL="$L" grep -c '[^[:print:][:space:]]' $data/text)
    if [ $n_non_print -ne 0 ]; then
      echo "$0: $data/text contains $n_non_print lines with non-printable characters. Attempting to clean..."
      LC_ALL="$L" sed -i 's/[^[:print:][:space:]]//g' $data/text
      n_non_print=$(LC_ALL="$L" grep -c '[^[:print:][:space:]]' $data/text)
      if [ $n_non_print -ne 0 ]; then
        echo "$0: $data/text still contains $n_non_print lines with non-printable characters after cleaning. Exiting..."
        exit 1;
      fi
    fi
  fi
