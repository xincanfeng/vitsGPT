...
with open("vitsGPT/datasets/EmoV_DB_bea_filtered/metadata.csv", "r") as meta_file:
    meta_dict = {
        line.split("|")[1].strip(): line.split("|")[0] for line in meta_file.readlines()
    }
# 处理 text 文件
with open(
    f"vitsGPT/datasets/EmoV_DB_bea_filtered/emovdb-bea_audio_emo_{i}.txt", "r"
) as text_file:
    lines = text_file.readlines()
with open(
    f"vitsGPT/datasets/EmoV_DB_bea_filtered/audio_llama-emo_text_{i}.txt", "w"
) as output_file:
    for line in lines:
        pre, post = line.split("|")
...
# 获取所有文件内容
all_files_content = [
    read_file_content(os.path.join(directory_path, filename))
...
for i in range(len(all_files_content[0])):
    ...
    if len(set(second_column_values)) > 1:
        print(
            f"Inconsistency detected in emotions for line {first_column_values[0]}: {second_column_values}"
        )
        inconsistency_found = True
        break
...
def filter_and_copy_wavs(
    text_file, filtered_text_file, source_directory, destination_directory
):
    # 读取文本文件内容
    ...
    for wav_file in all_wav_files:
        ...
        os.rename(
            os.path.join(source_directory, wav_file),
            os.path.join(source_directory, normalized_name),
        )
    unmatched_ids = []
    with open(filtered_text_file, "w") as out_file:
        for line in lines:
            ...
            matching_files = [
                f
                for f in get_all_wav_files(source_directory)
                if f.endswith(f"_{id}.wav") and emotion.lower() in f
            ]
            # 检查是否有唯一匹配的wav文件
            if len(matching_files) == 1:
                shutil.copy(
                    os.path.join(source_directory, matching_files[0]),
                    os.path.join(destination_directory, matching_files[0]),
                )
    ...
    if unmatched_ids:
        print(
            f"Unmatched or multiple matches found for IDs: {', '.join(unmatched_ids)}"
        )
...
filtered_text_file = (
    "vitsGPT/datasets/EmoV_DB_bea_filtered/audio_llama-emo_wav_filtered.txt"
)
...
print(
    f"Total duration of all WAV files in filtered_text: {total_duration} seconds ({total_duration/60} minites)"
)
print(
    f"Train duration of all WAV files in filtered_text: {train_duration} seconds ({train_duration/60} minites)"
)
print(
...
def split_dataset_maintain_ratio(
    filtered_text_file, train_file, dev_file, test_file, dev_size=50, test_size=50
):
    # 读取filtered_text文件
    with open(filtered_text_file, "r") as f:
        lines = f.readlines()
